...
The so called 'triplet bubble sort' is just sort all the number with odd index and then sort all the numbers with even index. 
= =||| There is no need to care about that triplet bubble sort.

```
def rescueTroubleSort(num_of_case):
    for n in range(int(num_of_case)):
        N = input()
        N = int(N)
        L = input().strip().split(' ')
        L = [int(i) for i in L]
...
num_of_case = input().strip()
rescueTroubleSort(num_of_case)
```
